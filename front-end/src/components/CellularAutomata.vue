...
export default {
	...
	methods: {
		...
		initializeCanvas() {
			...
			let worldSetup = this.$root.$data.world
				? this.$root.$data.world
				: this.defaultWorld;
			...
			const sketch = (s) => {
				...
				let graph = {
					size: worldSetup.size,
					states: worldSetup.numOfStates,
					ruleNum: parseInt(worldSetup.rule),
					styleType: "default",
					colors: worldSetup.colors,
					initialType: worldSetup.initialType,
					running: false,
					vector: [],
					init: function () {
						let row = [];
						...
						if (this.initialType == "center") {
							...
							for (let i = 0; i < this.size; i++) {
								row.push(
									Math.floor(Math.random() * this.states)
								);
							}
						}
